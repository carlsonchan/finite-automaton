...
describe('FiniteAutomaton implementing mod three', () => {
    beforeAll(() => {
        fsm = new FiniteAutomaton(states, inputSymbol, initialState, acceptingStates, transitionStates);
    })
    afterEach(() => {
    ...
    test('when invalid input is provided then throw', () => {
        expect(() => fsm.run("2")).toThrow("Unable to move from S0 with input of 2 to next state");
    })
})

describe('FiniteAutomaton validation', () => {
    test('when invalid initial state is set then throw error', () => {
        expect(() => new FiniteAutomaton(states, inputSymbol, "invalidState", acceptingStates, transitionStates)).toThrow("Initial state is not part of states");
    })
    test('when accepting state is not a subset of states then throw error', () => {
        const newAcceptingState = new Set([...acceptingStates, "s4"]);
        expect(() => new FiniteAutomaton(states, inputSymbol, initialState, newAcceptingState, transitionStates)).toThrow("Accepting/final states is not a subset of states");
    })
    test('when keys from transition state do no not match with the states then throw error', () => {
        const newState = new Set([...states, "S4"]);
        expect(() => new FiniteAutomaton(newState, inputSymbol, initialState, acceptingStates, transitionStates)).toThrow("States are missing or transition states are missing");
    })
    test('when input symbols are missing from the transition state then throw error', () => {
        const newInputSymbol = new Set([...inputSymbol, "2"]);
        expect(() => new FiniteAutomaton(states, newInputSymbol, initialState, acceptingStates, transitionStates)).toThrow("Input symbols are missing or transition states have extra symbols");
    })
    test('when multple input parameter are incorrect then throw all related errors', () => {
        const newInputSymbol = new Set([...inputSymbol, "2"]);
        expect(() => new FiniteAutomaton(states, newInputSymbol, "invalidState", acceptingStates, transitionStates)).toThrow("Initial state is not part of states, Input symbols are missing or transition states have extra symbols");
    })
})